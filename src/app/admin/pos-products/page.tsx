...
export default function AdminPosProductsPage() {
  ...
  const products: Product[] =
    productsData?.data?.map((apiProduct) => ({
      id: apiProduct.id,
      name: apiProduct.name,
      price: apiProduct.price,
      deposit: 0, // Default deposit since API doesn't have it
      description: apiProduct.category || "",
      stock:
        apiProduct.stock_quantity === 0 ? 0 : apiProduct.stock_quantity || "",
      image: apiProduct.image_url || "/images/place-holder.png",
      hidden: !apiProduct.is_active,
    })) || [];

  const loading = productsLoading || terminalsLoading;
  ...
  useEffect(() => {
    // Load POS mappings from localStorage (backward compatibility)
    const mappings = JSON.parse(localStorage.getItem("posMappings") || "[]");
    setPosProductMappings(mappings);
  }, []);

  // Initialize POS mappings if empty
  useEffect(() => {
    if (
      posTerminals &&
      posTerminals.length > 0 &&
      posProductMappings.length === 0
    ) {
      const defaultMappings = posTerminals.map((terminal) => ({
        posId: terminal.id,
        productIds: products.map((p) => p.id), // Assign all products to all terminals by default
      }));
      setPosProductMappings(defaultMappings);
      localStorage.setItem("posMappings", JSON.stringify(defaultMappings));
    }
  }, [posTerminals, products, posProductMappings]);
  ...
  const handleAddProduct = async (newProduct: Product) => {
    try {
      // The product is already created via API in the modal
      ...
      localStorage.setItem("posMappings", JSON.stringify(updatedMappings));
  ...
  const handleUpdateProduct = async (
    ...
    try {
      // The product is already updated via API in the modal
      // Just refetch to get the latest data
      await refetchProducts();
  ...
  const handleDeleteProduct = async (productId: string) => {
    if (confirm("Are you sure you want to delete this product?")) {
      try {
        ...
        alert(`Failed to delete product: ${handleApiError(error)}`);
      }
    }
  };
  ...
  const handleToggleProductVisibility = async (productId: string) => {
    ...
    try {
      ...
      const success = await updateProduct(productId, {
        is_active: !updatedHidden, // API uses is_active instead of hidden
      });
      ...
      alert(`Failed to update product: ${handleApiError(error)}`);
    }
  };
    ...
    try {
      ...
      for (const productData of importedProducts) {
        const apiProduct = await createProduct({
          ...
          image_url:
            productData.image !== "/images/place-holder.png"
              ? productData.image
              : undefined,
          category: productData.description || undefined,
          stock_quantity:
            productData.stock === "" ? undefined : Number(productData.stock),
        });

        if (apiProduct) {
          newProductIds.push(apiProduct.id);
        }
      }
      ...
      const updatedMappings = posProductMappings.map((mapping) =>
        mapping.posId === selectedPosId
          ? {
              ...mapping,
              productIds: [...mapping.productIds, ...newProductIds],
            }
          : mapping
      );
      ...
      alert(`Failed to import products: ${handleApiError(error)}`);
    }
  };
              ...
              <div className="flex flex-col gap-4">
                ...
                <div>
                  <h2 className="text-lg sm:text-xl font-semibold text-gray-900">
                    {selectedPos?.terminal_name} Products ({posProducts.length})
                  </h2>
                  <p className="text-sm text-gray-600 mt-1">
                    {productCounts.visible} visible, {productCounts.hidden}{" "}
                    hidden
                  </p>
                </div>
                      ...
                      onClick={async () => {
                        if (
                          ...
                          try {
                            ...
                            const deletePromises = selectedProducts.map(
                            ...
                            alert(
                              `Some products could not be deleted: ${handleApiError(
                                error
                              )}`
                            );
                          }
            ...
            {posProducts.length === 0 ? (
              <div className="p-6 sm:p-12 text-center">
                ...
                <h3 className="text-base sm:text-lg font-medium text-gray-900 mb-2">
                  No products in {selectedPos?.terminal_name}
                </h3>
                        ...
                        >
                          ...
                          <td className="px-4 sm:px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center">
                              <Image
                                src={
                                  product.image || "/images/place-holder.png"
                                }
                                alt={product.name}
                                width={48}
                                height={48}
